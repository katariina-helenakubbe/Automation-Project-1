...
describe('This is first test suite, Katariina-Helena Kubbe' , () => {
    //19.11.2023
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Katariina')
        cy.get('#lastName').type('Kubbe')
        cy.get('input[name="password"]').type('Arvuti123')
        cy.get('[name="confirm"]').type('Arvuti123')
        cy.get('#username').type('KatariinaTest')
    ...
    */
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').type('SeePoleNumber')
        cy.get("input[name='password']").type('jah')
        cy.get('[name="confirm"]').type('jah')
